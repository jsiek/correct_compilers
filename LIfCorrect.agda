...
--   where
--   t = If op a₁ a₂ e₁ e₂
-- create-block-correct (Goto lbl) B B' lbl' ρ s r refl t⇓ = 
--   goto-⇓ (nth-++-1 B t) (eval-tail-blocks t ρ B (B ++ [ t ]) s (r .proj₂) (r .proj₁)
--            ([ t ] , refl) t⇓)
--   where
--   t = Goto lbl
...
explicate-tail-correct (Atom a) ρ B B' t s r refl ie = return-⇓ ie
explicate-tail-correct Read ρ B B' t s r refl ie = return-⇓ ie
explicate-tail-correct (Uni op a) ρ B B' t s r refl ie = return-⇓ ie
explicate-tail-correct (Bin op a₁ a₂) ρ B B' t s r refl ie = return-⇓ ie
...
... | nothing | ()
... | just (v₁ , s1) | ie2
    with explicate-tail-correct e₂ (update ρ x v₁) B B₁ t₂ s1 r et2 ie2
... | t₂⇓r
    =
    ...
    explicate-assign-correct x e₁ t₂ t ρ B₁ B' s s1 v₁ r et ie1 B'⊢t₂⇓r
...
... | just (Bool true , s1) | ie2 =
    let t₂⇓r = explicate-tail-correct e₂ ρ B B₁ t₂ s1 r et2 ie2 in
...
... | just (Bool false , s1) | ie2 =
    let t₃⇓r = explicate-tail-correct e₃ ρ B₁ B₂ t₃ s1 r et3 ie2 in
    let B₂↝B' = explicate-pred-blocks e₁ t₂ t₃ t B₂ B' et in
...
... | false
    ...
    cond rewrite ia1 | ia2 = ie
explicate-pred-correct (Assign x e₁ e₂) t₁ t₂ t₃ ρ B₄ B₅ s s1 r b ep ie eb = {!!}
