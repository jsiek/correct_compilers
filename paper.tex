...
\subsection{Remove Complex Operands is Correct}
...
\subsection{Lift Locals is Correct}
...
grammar of \lstinline{Imp-Exp} (section~\ref{sec:imp}), there are
...
\subsection{Explicate Control is Correct}
...
The case for conditional expressions also requires a lemma about the
...
G$₁$ $↝$ G$₂$ = $∃$[ G ] G$₁$ ++ G $≡$ G$₂$
\end{lstlisting}
\noindent We prove the following lemma which states that the ouput CFG of
\lstinline{explicate-tail} is an extension of its input CFG.
\begin{lstlisting}
...
\noindent The case for conditional expressions needs a couple more
lemmas regarding control flow graphs. Similar to
\lstinline{explicate-tail}, we need a lemma which states that the
ouput CFG of \lstinline{explicate-assign} is an extension of its input
CFG.

\begin{lstlisting}
explicate-assign-CFG : ∀{x}{m}{t t'}{G₁ G₂}
  → explicate-assign x m t G₁ ≡ (t' , G₂)
  → G₁ ↝ G₂
\end{lstlisting}
