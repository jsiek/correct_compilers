...
explicate-assign-correct : ∀(x : Id)(e : IL-Exp)(t : CStmt) (ρ ρ′ ρ″ : Env Value) (s s′ s″ : Inputs)
    (n v : Value)
  → (s , ρ) ⊢ e ⇓ n ⊣ (s′ , ρ′)
...
explicate-assign-correct x (Sub a₁ a₂) t ρ ρ′ ρ″ s s′ s″ n v (⇓sub{n₁ = n₁}{n₂} ia₁ ia₂ subv) t⇓v =
    ⇓assign Goal t⇓v
    where
...
explicate-tail-correct : ∀ (e : IL-Exp) (ρ ρ' : Env Value) (s s' : Inputs) (v : Value)
  → (s , ρ) ⊢ e ⇓ v ⊣ (s' , ρ')
...
explicate-tail-correct (Sub a₁ a₂) ρ ρ' s s' v (⇓sub{n₁ = n₁}{n₂} ia1 ia2 subv) = ⇓return Goal
...
explicate-correct (Program n e) s v ((s' , ρ') , e⇓v) =
    ((s' , ρ')) , explicate-tail-correct e (replicate n (Int 0ℤ)) ρ' s s' v e⇓v
